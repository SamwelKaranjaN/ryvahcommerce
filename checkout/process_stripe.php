...
try {
    ...
    $charge = \Stripe\Charge::create([
        ...
        'currency' => 'usd',
        'source' => $data['token'],
        'description' => 'Ryvah Commerce Purchase'
    ]);

    if ($charge->status === 'succeeded') {
        // Update order status
        $sql = "UPDATE orders SET payment_status = 'completed', payment_method = 'stripe' WHERE id = ?";
        ...
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("si", $charge->id, $order_id);
        $stmt->execute();

        // Add order status history
        $sql = "INSERT INTO order_status_history (order_id, status, notes) VALUES (?, 'completed', 'Payment completed via Stripe')";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $order_id);
        $stmt->execute();
