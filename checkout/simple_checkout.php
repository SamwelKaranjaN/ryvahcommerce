...
try {
    ...
    // Calculate tax based on default address or first address
    $default_address = $addresses[0] ?? null;
    $tax_rate = 0;
    $tax_amount = 0;

    if ($default_address) {
        // Get product types for tax calculation
        $product_types = [];
        foreach ($cart_items as $item) {
            ...
            if ($result->num_rows > 0) {
                $product_types[$item['id']] = $result->fetch_assoc()['type'];
            }
        }

        // Calculate tax for each item
        foreach ($cart_items as $item) {
            $product_type = $product_types[$item['id']] ?? null;
            $item_tax_rate = getTaxRate($default_address['state'], $default_address['country'], $product_type);
            $item_tax = ($item['price'] * $item['quantity']) * $item_tax_rate;
            $tax_amount += $item_tax;
        }
    }
...
<head>
    ...
    <meta http-equiv="Content-Security-Policy"
        content="default-src 'self'; script-src 'self' 'unsafe-inline' https://www.paypal.com https://www.sandbox.paypal.com https://*.paypal.com https://cdn.jsdelivr.net https://cdnjs.cloudflare.com ws://gc.kis.v2.scr.kaspersky-labs.com; style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://cdnjs.cloudflare.com ws://gc.kis.v2.scr.kaspersky-labs.com; font-src 'self' https://cdn.jsdelivr.net https://cdnjs.cloudflare.com; connect-src 'self' https://api.sandbox.paypal.com https://api.paypal.com https://www.sandbox.paypal.com https://www.paypal.com https://*.paypal.com; frame-src 'self' https://www.paypal.com https://www.sandbox.paypal.com https://*.paypal.com ws://gc.kis.v2.scr.kaspersky-labs.com; child-src 'self' https://www.paypal.com https://www.sandbox.paypal.com https://*.paypal.com; img-src 'self' data: https: blob:;">
    ...
    <script
        src="https://www.paypal.com/sdk/js?client-id=<?= htmlspecialchars(PAYPAL_SANDBOX_CLIENT_ID); ?>&currency=<?= htmlspecialchars($currency); ?>&intent=capture&enable-funding=venmo,paylater"
        defer></script>
...
<body>
                ...
                <div class="order-summary">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex justify-content-between mb-2">
                                <span>Tax:</span>
                                <span id="tax-display"><?= formatCurrency($tax_amount, $currency); ?></span>
                            </div>
                            <hr>
                            <div class="d-flex justify-content-between mb-3">
                                <span class="h6">Total:</span>
                                <span class="total-amount"
                                    id="total-display"><?= formatCurrency($total, $currency); ?></span>
                            </div>
    ...
    let currentTax = <?= json_encode($tax_amount); ?>;
    ...
    // Update order totals
    function updateOrderTotals(newTax, newTotal) {
    ...
    // Calculate tax based on selected address
    function calculateTax(state, country) {
        // This would typically be an AJAX call to calculate tax
        // For now, using client-side approximation
        ...
        if (country === 'US') {
            ...
            taxRate = stateTaxRates[state] || 0.08;
        }
    ...
    document.getElementById('address-select').addEventListener('change', function() {
        ...
        if (this.value) {
            ...
            // Calculate new tax
            const newTax = calculateTax(state, country);
            const newTotal = subtotal + newTax;

            updateOrderTotals(newTax, newTotal);
