...
function createPayPalOrder($orderData, $address, $currency)
{
    try {
        ...
        // Use enhanced client with SSL bypass for development
        if (class_exists('RyvahPayPalHttpClient')) {
            $client = new RyvahPayPalHttpClient($environment);
        } else {
        ...
        if ($shouldBypass) {
            logPayPalError('Using PayPal client with SSL bypass for development');
        }
